...
export class AnalyticsEngine {
  ...
  constructor(dataDir?: string) {
    ...
    if (dataDir && process.env.PERSIST_ANALYTICS !== 'false') {

      this.persistPath = `${dataDir}/analytics/events.jsonl`;
  ...
  track(event: Omit<AnalyticsEvent, 'at'>): void {
    ...
    if (this.persistPath) {
      ...
      void import('./file-store.js').then(({ appendJsonl, ensureDir }) => {
        ...
        return ensureDir(dir).then(() => appendJsonl(this.persistPath!, safe));
